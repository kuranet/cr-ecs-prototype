    ...
    public partial struct WanderSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                var currentVal = animator.GetBool(isMovingID);
                animator.SetBool(isMovingID, isMoving);
                if(currentVal != isMoving)
                {
                    UnityEngine.Debug.LogError($"change animation val to {isMoving}");
                }
            }
                     ...
                     SystemAPI.Query<RefRW<LocalTransform>, RefRW<WanderState>>())
            {
                ...
                else
                {
                    ...
                    if (isMoving)
                    {
                        ...
                        UnityEngine.Debug.LogError($"update position {transform.ValueRO.Position}");
                    }
                }
            }
        }
